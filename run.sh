...
if [[ ! -f ${ROOTFS_DIR}/usr/bin/gcc-7 ]]; then
  ...
  chroot ${ROOTFS_DIR} /bin/bash -c 'apt-get -t buster --no-upgrade -y install linux-libc-dev gcc-7'
...
# Blacklist nouveau to ensure clean reboots
if [[ ! -f ${ROOTFS_DIR}/etc/modprobe.d/nvidia-installer-disable-nouveau.conf  ]]; then
    cat > ${ROOTFS_DIR}/etc/modprobe.d/nvidia-installer-disable-nouveau.conf <<EOF
...
if [[ ! -f ${ROOTFS_DIR}/etc/apt/sources.list.d/nvidia-docker.list ]]; then
    # Trust NVIDIA key
    curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey | chroot ${ROOTFS_DIR} apt-key add -
    
    # Install NVIDIA repo
    distribution=$(. /etc/os-release;echo $ID$VERSION_ID)
    curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list \
    | chroot ${ROOTFS_DIR} tee /etc/apt/sources.list.d/nvidia-docker.list
    
    # Install nvidia-docker
    chroot ${ROOTFS_DIR} apt-get update
    chroot ${ROOTFS_DIR} apt-get install nvidia-docker2=2.0.3+docker17.03.2-1 nvidia-container-runtime=2.0.0+docker17.03.2-1 -y
...
EOF
    ...
    chroot ${ROOTFS_DIR} systemctl reload docker.service
fi


# Restart Kubelet
echo "Restarting Kubelet"
chroot ${ROOTFS_DIR} systemctl restart kubelet.service